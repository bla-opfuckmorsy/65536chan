...
function decodePost(jsonObj,setID)
{
	...
	//Useless shit for now
	/*if(jsonObj.replysto.length != 0)
	{
		replyarray[jsonObj.postnum] = jsonObj.replysto;
	}*/
...
//also useless shit
/*function processReplys(replys, postnumber)
{
	for(var j = 0; j < replys.length; ++j)
	{
		var post = document.getElementById("pnspan_"+replys[j]);
		if(post)
		{
			//post.innerHTML += "  <span style=\"font-size:small;color:#0000ff;\" onclick=\"gotoPost("+postnumber+")\" onmouseover=\"hoverPost("+postnumber+", event)\" onmouseout=\"unhoverPost()\">&gt;&gt;"+postnumber+"</span>";
		}
	}
}*/

//replyarray is useless shit for now
var replyarray;
	...
	xmlhttp.onreadystatechange = function()
	{
		if(xmlhttp.readyState == 4 && xmlhttp.status == 200)
		{
			if(xmlhttp.responseText.substr(0,6) != "Error:")
			{
				...
				/*for(key in replyarray)
				{
					processReplys(replyarray[key],key);
				}*/
			} else
			{
				//Error message here.
